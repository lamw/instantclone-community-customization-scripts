Import-Module C:\Users\primp\Desktop\InstantClone.psm1
...
#>
    ...
    if($StringInput -ne "") {
        ...
        $hidCodeHexToInt = [Convert]::ToInt64("0x58","16")
    ...
    ##Write-Host "Sending keystroke to $VMName ...`n"
...
foreach ($i in 1..$numOfVMs) {
    ...
    Write-Host "Hot Adding Flexible Network Adapter ..."
    New-NetworkAdapter -VM $vm -NetworkName "VM Network" -Type Flexible -StartConnected | Out-Null
    Write-Host "Sending keystrokes to configure adapter ..."
    Start-Sleep -Seconds 2
    Set-VMKeystrokes -VMName $VM -StringInput ""
    ...
    Start-Sleep -Seconds 1
    Set-VMKeystrokes -VMName $VM -StringInput ""
    Write-Host "Sleeping for 23 seconds waiting for adapter to be ready ..."
    Start-Sleep -Seconds 23
    Set-VMKeystrokes -VMName $VM -StringInput "N"
}
